...
public interface ApplicationModulesParametersProvider {
    static Optional<ApplicationModuleParameters> getApplicationModuleParameters(ModuleKey moduleKey) {
        ...
        return getConfig(moduleKey.formatKey())
                .map(Value::asEntity)
                .map(moduleEntity -> moduleEntity.getString(APPLICATION_MODULE_PROTOBUF_SERVLET_PATH_CONFIG_KEY))
                .map(servletPath -> new ApplicationModuleParameters(balancerHost, balancerPort, servletPath));
    }
